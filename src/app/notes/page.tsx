  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-24 md:px-8">
      <header className="mb-14 border-b border-border pb-10">
        ...
        <h1 className="mt-4 max-w-4xl text-balance text-5xl font-medium tracking-tight md:text-7xl">{settings.notesTitle}</h1>
        <p className="mt-5 max-w-2xl text-lg leading-8 text-muted-foreground">{settings.notesDescription}</p>
      </header>
      <PostList posts={posts} emptyText={settings.notesEmptyText} />
      <ContactFooter />
    </main>
  );
}
